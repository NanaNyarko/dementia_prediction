...
def predict(image_path):
    ...
    class_dictionary = np.load(
        "trained_models/oasis_cross-sectional_class_indices" + "_" + data_type + ".npy",
        allow_pickle=True,
    ).item()
    ...
    orig = imutils.resize(
        orig, width=600
    )  # Make images bigger (training data is in high-quality format)
    ...
    print(label)
    cv2.putText(orig, label, (20, 45), cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 255, 0), 2)
...
if __name__ == "__main__":
    ...
    top_model_weights_path = (
        "trained_models/oasis_cross-sectional" + "_" + data_type + ".h5"
    )
    path = args["file"]
    send_from_dir(path)
    cv2.destroyAllWindows()
